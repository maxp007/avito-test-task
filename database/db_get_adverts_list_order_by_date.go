...
func Db_get_adverts_list_order_by_date(page int64, order_date string) (response []models.AdvertListElement, err error) {
	...
	rows, err := Pool.Query(context.Background(), "select * FROM adverts_schema.getadvertslistorderedbydate($1,$2,$3);", int32(page), int32(ads_per_page), order_date)
	if err != nil {
		log.Println(err)
	}
